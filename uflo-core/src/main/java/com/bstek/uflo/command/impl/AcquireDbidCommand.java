...
public class AcquireDbidCommand implements Command<Long>{
	...
	public Long execute(Context context) {
		...
		List<ContextProperty> list=session.createQuery("from "+ContextProperty.class.getName()+" as p where p.key=:key").setLockMode("p", LockMode.PESSIMISTIC_WRITE).setString("key", ID_KEY).list();
		if(list.size()>0){
